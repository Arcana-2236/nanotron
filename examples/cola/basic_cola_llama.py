...
class ColaLayer(nn.Module):
    def __init__(
        ...
        # Log CoLA layer creation statistics
        dense_params = in_features * out_features + (out_features if bias else 0)
        cola_params = in_features * rank + rank * out_features + (out_features if bias else 0)
        compression_ratio = dense_params / cola_params if cola_params > 0 else 0

        log_rank(
            f"ColaLayer created: in={in_features}, out={out_features}, rank={rank}, "
            f"lr_act={lr_act_type if lr_act else 'None'}, bias={bias}, "
            f"dense_params={dense_params:,}, cola_params={cola_params:,}, "
...
class MLP(nn.Module):
    def __init__(
        self,
        config: BasicColaLlamaConfig,
        parallel_config: Optional[ParallelismArgs],
        tp_pg: dist.ProcessGroup,
    ):
        super().__init__()

        assert config.mlp_rank is not None, "mlp_rank must be set in ColaLlamaConfig for CoLA model"
        self.both_act = config.both_act

        # CoLA layers for MLP
        self.gate_proj = ColaLayer(
            in_features=config.hidden_size,
            out_features=config.intermediate_size,
            rank=config.mlp_rank,
            bias=False,
    ...
    def forward(self, hidden_states):  # [seq_length, batch_size, hidden_dim]
        ...
        hidden_states = (self.lr_act(gate_states) if self.both_act else gate_states) * up_states
        hidden_states = self.down_proj(hidden_states)
        return {"hidden_states": hidden_states}


class CoreAttention(nn.Module):
    def __init__(self, config: BasicColaLlamaConfig, parallel_config: Optional[ParallelismArgs], layer_idx: int):
        super().__init__()
...
class CausalSelfAttention(nn.Module, AttachableStore):
    def __init__(
        ...
        # No tensor parallelism for basic CoLA implementation
        try:
        ...
        assert config.attn_rank is not None, "attn_rank must be set in ColaLlamaConfig for CoLA model"

        # CoLA layers for attention projections
...
class BasicColaLlamaDecoderLayer(nn.Module):
    def __init__(
        self,
        config: BasicColaLlamaConfig,
        parallel_config: Optional[ParallelismArgs],
...
class Embedding(nn.Module, AttachableStore):
    def __init__(self, tp_pg: dist.ProcessGroup, config: BasicColaLlamaConfig, parallel_config: Optional[ParallelismArgs]):
        super().__init__()
...
class BasicColaLlamaModel(nn.Module):
    ...
    def __init__(
        ...
        self.decoder = nn.ModuleList(
            [
                PipelineBlock(
                    p2p=self.p2p,
                    module_builder=BasicColaLlamaDecoderLayer,
                    module_kwargs={
                        "config": config,
                        "parallel_config": parallel_config,
                        "tp_pg": parallel_context.tp_pg,
    ...
    def get_block_compute_costs(self):
        ...
        block_compute_costs = {
            ...
            BasicColaLlamaDecoderLayer: 4 * model_config.num_attention_heads * d_qkv * model_config.hidden_size
...
class Loss(nn.Module):
    def __init__(self, tp_pg: dist.ProcessGroup):
        super().__init__()
        # No TP for basic CoLA implementation
    ...
    def forward(
        ...
        # Use standard cross entropy instead of sharded cross entropy
        # Convert logits to [batch_size, vocab_size, seq_length] for cross_entropy
        logits = sharded_logits.transpose(0, 1)  # [batch_size, seq_length, vocab_size]
        logits = logits.transpose(1, 2)  # [batch_size, vocab_size, seq_length]
...
class BasicColaLlamaForTraining(NanotronModel):
    def __init__(
        self,
        config: BasicColaLlamaConfig,
        parallel_context: ParallelContext,
        ...
        super().__init__()
        self.model = BasicColaLlamaModel(config=config, parallel_context=parallel_context, parallel_config=parallel_config)
        ...
        # Log aggregate CoLA statistics
        self._log_cola_statistics()

    def _log_cola_statistics(self):
        """Log aggregate statistics for all CoLA layers in the model."""
        total_cola_params = 0
        ...
        if num_cola_layers > 0:
            overall_compression = total_dense_params / total_cola_params if total_cola_params > 0 else 0
            log_rank(
                f"\n{'='*80}\n"
                f"CoLA Model Statistics:\n"
                f"  Number of CoLA layers: {num_cola_layers}\n"
                f"  Total CoLA parameters: {total_cola_params:,}\n"
                f"  Equivalent dense parameters: {total_dense_params:,}\n"
                f"  Parameters saved: {total_dense_params - total_cola_params:,}\n"
