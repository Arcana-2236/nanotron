...
"""PyTorch CoLA LLaMa model with vanilla TP."""
...
class MLP(nn.Module):
    def __init__(
        ...
        gate_up_contiguous_chunks = (
            config.mlp_rank,  # shape of gate_linear
            config.mlp_rank,  # shape of up_linear
        )
        self.gate_up_proj0 = TensorParallelColumnLinear(
            config.hidden_size,
            2 * config.mlp_rank,
            pg=tp_pg,
            mode=tp_mode,
        ...
        self.gate_proj1 = TensorParallelRowLinear(
            config.mlp_rank,
        ...
        self.up_proj1 = TensorParallelRowLinear(
            config.mlp_rank,
            config.intermediate_size,
            pg=tp_pg,
            mode=tp_mode,
        ...
        self.down_proj0 = TensorParallelColumnLinear(
            config.intermediate_size,
            config.mlp_rank,
            pg=tp_pg,
            mode=tp_mode,
        ...
        self.down_proj1 = TensorParallelRowLinear(
            config.mlp_rank,
            config.hidden_size,
            pg=tp_pg,
            mode=tp_mode,
    ...
    def forward(self, hidden_states):  # [seq_length, batch_size, hidden_dim]
        merged_lr_states = self.gate_up_proj0(hidden_states)    # [seq_length, batch_size, 2 * config.mlp_rank // TP]
        merged_lr_states = self.lr_act(merged_lr_states)        # [seq_length, batch_size, 2 * config.mlp_rank // TP]
        lr_gate_states, lr_up_states = torch.split(merged_lr_states, merged_lr_states.shape[-1] // 2, dim=-1) # [seq_length, batch_size, config.mlp_rank // TP]
        gate_states = self.gate_proj1(lr_gate_states)            # [seq_length, batch_size, config.intermediate_size]
        up_states = self.up_proj1(lr_up_states)                  # [seq_length, batch_size, config.intermediate_size]

        hidden_states = gate_states * up_states                  # [seq_length, batch_size, config.intermediate_size]
        lr_down_states = self.down_proj0(hidden_states)          # [seq_length, batch_size, config.mlp_rank // TP]
        lr_down_states = self.lr_act(lr_down_states)             # [seq_length, batch_size, config.mlp_rank // TP]
        hidden_states = self.down_proj1(lr_down_states)          # [seq_length, batch_size, config.hidden_size]
...
class CausalSelfAttention(nn.Module, AttachableStore):
    def __init__(
        ...
        qkv_contiguous_chunks = (
            config.attn_rank,  # shape of q
            config.attn_rank,  # shape of k
            config.attn_rank,  # shape of v
        )
        self.lr_act = ACT2FN[config.hidden_act]
        self.qkv_proj0 = TensorParallelColumnLinear(
            self.d_model,
            config.attn_rank * 3,
            pg=tp_pg,
            mode=tp_mode,
        ...
        self.q_proj1 = TensorParallelRowLinear(
            config.attn_rank,
        ...
        self.k_proj1 = TensorParallelRowLinear(
            config.attn_rank,
        ...
        self.v_proj1 = TensorParallelRowLinear(
            config.attn_rank,
        ...
        self.o_proj0 = TensorParallelColumnLinear(
            config.num_attention_heads * self.d_qk,
            config.attn_rank,
            pg=tp_pg,
            mode=tp_mode,
        ...
        self.o_proj1 = TensorParallelRowLinear(
            config.attn_rank,
            self.d_model,
            pg=tp_pg,
            mode=tp_mode,
    ...
    def forward(
        ...
        lr_qkv_states = self.qkv_proj0(hidden_states)   # [seq_length, batch_size, config.attn_rank * 3]
        lr_qkv_states = self.lr_act(lr_qkv_states)      # [seq_length, batch_size, config.attn_rank * 3]
        lr_q_states, lr_k_states, lr_v_states = torch.split(lr_qkv_states, lr_qkv_states.shape[-1] // 3, dim=-1) # [seq_length, batch_size, config.attn_rank]
        query_states = self.q_proj1(lr_q_states)         # [seq_length, batch_size, config.num_attention_heads * self.d_qk]
        key_states = self.k_proj1(lr_k_states)            # [seq_length, batch_size, config.num_key_value_heads * self.d_qk]
        value_states = self.v_proj1(lr_v_states)          # [seq_length, batch_size, config.num_key_value_heads * self.d_qk]
        ...
        lr_output = self.o_proj0(attention_output)
        lr_output = self.lr_act(lr_output)
        output = self.o_proj1(lr_output)
        torch.cuda.nvtx.range_pop()
...
class VanillaColaLlamaForTraining(NanotronModel):
    def __init__(
        self,
        config: ColaLlamaConfig,
        parallel_context: ParallelContext,
