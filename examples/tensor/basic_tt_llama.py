...
class TensorTrainLinear(nn.Module):
    ...
    def __init__(
        ...
        log_rank(
            ...
            f"params={total_params:,}/{original_params:,} ({compression_ratio:.2f}x), std={target_std:.4f}",
            logger=logger,
            level=logging.INFO,
            rank=0,
        )
        ...
        if pg is not None and pg.size() > 1:
            raise ValueError(f"TP={pg.size()} not supported for TT")
...
class CausalSelfAttention(nn.Module, AttachableStore):
    def __init__(
        ...
        # No tensor parallelism for TensorTrain implementation
        try:
        ...
        # Use full heads instead of local heads (no TP)
        ...
        # TensorTrain layers for attention projections
...
class TTEmbedding(nn.Module, AttachableStore):
...
class TTLlamaModel(nn.Module):
    ...
    def __init__(
        ...
        self.decoder = nn.ModuleList(
            [
                PipelineBlock(
                    p2p=self.p2p,
                    module_builder=TTLlamaDecoderLayer,
                    module_kwargs={
                        "config": config,
                        "parallel_config": parallel_config,
                        "tp_pg": parallel_context.tp_pg,
    ...
    def get_block_compute_costs(self):
        ...
        block_compute_costs = {
            ...
            TTLlamaDecoderLayer: 4 * model_config.num_attention_heads * d_qkv * model_config.hidden_size
...
class TTLlamaForTraining(NanotronModel):
    def __init__(
        self,
        config: TTLlamaConfig,
        parallel_context: ParallelContext,
        ...
        super().__init__()
        self.model = TTLlamaModel(config=config, parallel_context=parallel_context, parallel_config=parallel_config)
        ...
        # Log aggregate TensorTrain statistics
        self._log_tt_statistics()

    def _log_tt_statistics(self):
        """Log aggregate statistics for all TensorTrain layers in the model."""
        total_tt_params = 0
        ...
        if num_tt_layers > 0:
            overall_compression = total_dense_params / total_tt_params if total_tt_params > 0 else 0
            log_rank(
                f"\n{'='*80}\n"
                f"TensorTrain Model Statistics:\n"
                f"  Number of TT layers: {num_tt_layers}\n"
                f"  Total TT parameters: {total_tt_params:,}\n"
                f"  Equivalent dense parameters: {total_dense_params:,}\n"
                f"  Parameters saved: {total_dense_params - total_tt_params:,}\n"
                f"  Overall compression ratio: {overall_compression:.2f}x\n"
                f"{'='*80}",
                logger=logger,
                level=logging.INFO,
                rank=0,
