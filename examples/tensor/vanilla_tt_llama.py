...
"""PyTorch Tensor-Train LLaMa model."""
...
from tt_utils import validate_tt_rank
...
class TensorTrainLinear(nn.Module):
    """Proper Tensor-Train decomposition for linear layers using 3D cores.

    Decomposes a weight matrix W: in_features × out_features into TT cores.
    Each core is a 3D tensor with shape (rank_in, mode_size, rank_out).

    The forward pass uses an efficient half-reconstruction strategy:
    1. Reconstruct first half of cores into a matrix
    2. Matrix multiply with input
    3. Reconstruct second half of cores
    4. Final matrix multiply

    Important: Uses nn.Parameter for 3D TT cores (not nn.Linear) because:
    1. True TT representation with 3D tensors
    2. More flexible and efficient than stacked Linear layers
    3. Still fully compatible with Pipeline Parallelism (PP)
    4. No TP needed - cores are replicated (small due to low rank)
    """
    def __init__(
        self,
        in_features: int,
        ...
        tt_rank: int,
        tt_cores: int = 4,
        tt_rank_ratio: float = 1.0,  # Rank ratio limit for TT validation
        pg: dist.ProcessGroup = None,  # Kept for API compatibility but not used
        mode: TensorParallelLinearMode = None,  # Kept for API compatibility but not used
        bias: bool = False,
        async_communication: bool = False,  # Kept for API compatibility but not used
        tp_recompute_allgather: bool = True,  # Kept for API compatibility but not used
        initializer_range: float = None,  # For initialization, falls back to 1/sqrt(fan_in + fan_out)
    ):
        super().__init__()

        self.in_features = in_features
        self.out_features = out_features
        self.tt_cores_count = tt_cores
        self.bias_enabled = bias

        # Compute mode sizes by factorizing dimensions
        self.input_modes = self._factorize_dimension(in_features, tt_cores//2, reverse=True)
        self.output_modes = self._factorize_dimension(out_features, tt_cores//2)
        self.tensor_modes = self.input_modes + self.output_modes

        # Validate and get TT ranks
        # Disallow overparameterization to ensure realizable ranks
        self.tt_ranks = validate_tt_rank(
            tensor_shape=self.tensor_modes,
            rank=tt_rank,
            constant_rank=True,  # Use uniform rank
            allow_overparametrization=False,  # Disallow overparameterization
            per_decomp_rank_ratio_limit=tt_rank_ratio,
        )

        # Compute target standard deviation for initialization
        # Default: 1/sqrt(fan_in + fan_out), or use provided initializer_range
        import math
        if initializer_range is not None:
            target_std = initializer_range
        else:
            fan_in = math.prod(self.input_modes)
            fan_out = math.prod(self.output_modes)
            target_std = math.sqrt(1.0 / (fan_in + fan_out))

        # Create TT cores as 3D tensors: (rank_in, mode_size, rank_out)
        # Each core handles one input mode and one output mode
        # Initialization follows Gaussian scheme: randn / (r1 * r2)^(1/4) * target_std^(1/order)
        self.cores = nn.ParameterList()
        std_per_core = target_std ** (1.0 / tt_cores)

        for i in range(tt_cores):
            rank_in = self.tt_ranks[i]
            rank_out = self.tt_ranks[i + 1]
            mode_size = self.tensor_modes[i]

            # Initialize core with proper Gaussian scaling
            # Scale by (r1 * r2)^(-1/4) to account for rank growth during reconstruction
            # Scale by target_std^(1/order) so that product of cores has target_std
            rank_scale = (rank_in * rank_out) ** (-0.25)
            init_std = rank_scale * std_per_core

            core = nn.Parameter(
                torch.randn(rank_in, mode_size, rank_out) * init_std
            )
            self.cores.append(core)

        # Log TT decomposition info (rank 0 only)
        total_params = sum(core.numel() for core in self.cores)
        original_params = in_features * out_features
        compression_ratio = 100 * total_params / original_params
        log_rank(
            f"TT-Linear({in_features}→{out_features}): ranks={self.tt_ranks}, "
            f"modes={self.input_modes}×{self.output_modes}, "
            f"params={total_params:,}/{original_params:,} ({compression_ratio:.1f}%), std={target_std:.4f}",
            logger=logger,
            level=logging.INFO,
            rank=0,
        )

        # Warn if TP > 1 since TT cores are replicated, not sharded (warn only once per shape)
        if pg is not None and pg.size() > 1:
            # Use class variable to track warned shapes
            if not hasattr(TensorTrainLinear, '_warned_tp_shapes'):
                TensorTrainLinear._warned_tp_shapes = set()

            shape_key = (in_features, out_features, pg.size())
            if shape_key not in TensorTrainLinear._warned_tp_shapes:
                TensorTrainLinear._warned_tp_shapes.add(shape_key)
                log_rank(
                    f"WARNING: TT-Linear({in_features}→{out_features}) with TP={pg.size()}: "
                    f"TT cores ({total_params:,} params) REPLICATED on all ranks (not sharded). "
                    f"Memory cost: {pg.size()}x replicated.",
                    logger=logger,
        ...
        # Precompute split point for half-reconstruction
        self.split_point = tt_cores // 2

    def _factorize_dimension(self, dim: int, num_factors: int, reverse: int = False, exact: bool = True, balanced: bool = True) -> list:
        """Factorize a dimension into num_factors factors.

        Args:
            dim: The dimension to factorize
            num_factors: Number of factors to produce
            reverse: Whether to reverse sort the factors
            exact: If True, ensure product of factors equals dim exactly
            balanced: If True, optimize for balanced factors (minimal variance)

        Returns:
            List of factors whose product equals dim (if exact=True)
        """
        import math
        ...
        if exact:
            if balanced:
                # Balanced exact factorization: minimize variance among factors
                ...
                def factorize_balanced_recursive(n, k, target):
                    """Recursively find most balanced factorization."""
                    if k == 1:
                        return [n], abs(n - target)

                    divisors = get_all_divisors(n)
                    best_factors = None
                    best_variance = float('inf')

                    # Try each divisor as the first factor
                    for d in divisors:
                        remaining = n // d
                        # Recursively factorize the remaining
                        sub_factors, _ = factorize_balanced_recursive(remaining, k - 1, target)
                        factors = [d] + sub_factors

                        # Calculate variance (measure of balance)
                        mean = sum(factors) / len(factors)
                        variance = sum((f - mean) ** 2 for f in factors)

                        if variance < best_variance:
                            best_variance = variance
                            best_factors = factors

                    return best_factors, best_variance

                # Target is geometric mean
                target = dim ** (1.0 / num_factors)
                factors, _ = factorize_balanced_recursive(dim, num_factors, target)
                return sorted(factors, reverse=reverse)
            else:
                # Exact factorization without balance optimization
                # Start with balanced approximation
                target = dim ** (1.0 / num_factors)
                factors = [int(round(target))] * num_factors

                # Adjust to make product exact
                current_prod = math.prod(factors)

                if current_prod != dim:
                    # Use greedy approach: try to find factors that multiply to exactly dim
                    factors = [1] * num_factors
                    remaining = dim

                    # Distribute factors greedily to keep them balanced
                    for i in range(num_factors):
                        # Calculate target factor for remaining slots
                        slots_left = num_factors - i
                        target_factor = int(round(remaining ** (1.0 / slots_left)))

                        # Find the best factor that divides remaining
                        best_factor = target_factor
                        for candidate in range(max(1, target_factor - 10), target_factor + 11):
                            if candidate > 0 and remaining % candidate == 0:
                                best_factor = candidate
                                break

                        # If no exact divisor found in range, find closest divisor
                        if remaining % best_factor != 0:
                            # Find divisors of remaining
                            divisors = []
                            for d in range(1, int(math.sqrt(remaining)) + 1):
                                if remaining % d == 0:
                                    divisors.append(d)
                                    if d != remaining // d:
                                        divisors.append(remaining // d)

                            # Choose divisor closest to target
                            best_factor = min(divisors, key=lambda x: abs(x - target_factor))

                        factors[i] = best_factor
                        remaining //= best_factor

                return sorted(factors, reverse=reverse)
        else:
            # Approximate balanced factorization (original behavior)
            factors = []
            remaining = dim
            for i in range(num_factors - 1):
                # Take the i-th root of remaining
                factor = max(1, int(round(remaining ** (1.0 / (num_factors - i)))))
    ...
    def forward(self, x):
        """Efficient forward pass using half-reconstruction.

        Strategy:
        1. Reconstruct first half of TT cores into matrix M1
        2. Compute intermediate = x @ M1
        3. Reconstruct second half of TT cores into matrix M2
        4. Compute output = intermediate @ M2

        This is faster than full reconstruction for large matrices.
        ...
        """
        # Step 1: Reconstruct first half of cores
        # This gives us a matrix of shape [prod(first_half_input_modes), middle_rank]
        first_half_matrix = self._reconstruct_half(0, self.split_point)

        # Step 2: First matrix multiply
        # [batch, first_half_input] @ [first_half_input, middle_rank] = [batch, middle_rank]
        intermediate = x @ first_half_matrix

        # Step 3: Reconstruct second half of cores
        # This gives us a matrix of shape [middle_rank, prod(second_half_output_modes)]
        second_half_output_size = int(torch.tensor(self.output_modes).prod().item())
        second_half_matrix = self._reconstruct_half(self.split_point, self.tt_cores_count).view(-1, second_half_output_size)

        # Step 4: Second matrix multiply
        # [batch, middle_rank] @ [middle_rank, second_half_output] = [batch, second_half_output]
        output = intermediate @ second_half_matrix
    ...
    def _reconstruct_half(self, start_idx: int, end_idx: int) -> torch.Tensor:
        """Reconstruct a portion of TT cores into a matrix.

        Args:
            start_idx: Starting core index (inclusive)
            end_idx: Ending core index (exclusive)
        ...
        result = self.cores[start_idx]  # (r_in, m_0, r_out)
        rank_in, mode_size, rank_out = result.shape

        # Reshape to (r_in * m_0, r_out) for matrix multiplication
        result = result.view(rank_in * mode_size, rank_out)

        # Multiply with subsequent cores
        for i in range(start_idx + 1, end_idx):
            core = self.cores[i]  # (r_in, m_i, r_out)
            r_in, m_i, r_out = core.shape

            # Reshape core to (r_in, m_i * r_out)
            core_reshaped = core.view(r_in, m_i * r_out)

            # Multiply: (prev_modes, r_in) @ (r_in, m_i * r_out) = (prev_modes, m_i * r_out)
            result = result @ core_reshaped

            # Reshape to (prev_modes * m_i, r_out)
            result = result.view(-1, r_out)

        return result


class MLP(nn.Module):
    """MLP with Tensor-Train decomposition for all linear layers.

    Standard SwiGLU architecture:
    out = down_proj(act(gate_proj(x)) * up_proj(x))

    But each projection uses TT decomposition.
    """
    def __init__(
        self,
        config: TTLlamaConfig,
        ...
        # Gate projection: hidden_size -> intermediate_size (via TT decomposition)
        self.gate_proj = TensorTrainLinear(
            in_features=config.hidden_size,
            out_features=config.intermediate_size,
            tt_rank=config.tt_rank,
            tt_cores=config.tt_cores,
            tt_rank_ratio=config.tt_rank_ratio,
            pg=tp_pg,
            mode=tp_mode,
            bias=False,
        ...
        # Up projection: hidden_size -> intermediate_size (via TT decomposition)
        self.up_proj = TensorTrainLinear(
            in_features=config.hidden_size,
            out_features=config.intermediate_size,
            tt_rank=config.tt_rank,
            tt_cores=config.tt_cores,
        ...
        # Down projection: intermediate_size -> hidden_size (via TT decomposition)
        self.down_proj = TensorTrainLinear(
    ...
    def forward(self, hidden_states):  # [seq_length, batch_size, hidden_size]
        """
        Standard SwiGLU MLP with Tensor-Train decomposition:
        out = down_proj(act(gate_proj(x)) * up_proj(x))
        """
        # Apply TT gate projection and activation
        gate_output = self.gate_proj(hidden_states)  # [seq_length, batch_size, intermediate_size]
        gate_output = self.act_fn(gate_output)

        # Apply TT up projection
        up_output = self.up_proj(hidden_states)  # [seq_length, batch_size, intermediate_size]

        # Element-wise multiplication (SwiGLU)
        hidden_states = gate_output * up_output  # [seq_length, batch_size, intermediate_size]

        # Apply TT down projection
        hidden_states = self.down_proj(hidden_states)  # [seq_length, batch_size, hidden_size]

        return {"hidden_states": hidden_states}


class CoreAttention(nn.Module):
    def __init__(self, config: TTLlamaConfig, parallel_config: Optional[ParallelismArgs], layer_idx: int):
        super().__init__()
...
class CausalSelfAttention(nn.Module, AttachableStore):
    def __init__(
        ...
        # Tensor-Train Q, K, V projections
        self.q_proj = TensorTrainLinear(
            in_features=self.d_model,
            out_features=config.num_attention_heads * self.d_qk,
            tt_rank=config.tt_rank,
            tt_cores=config.tt_cores,
            tt_rank_ratio=config.tt_rank_ratio,
            pg=tp_pg,
        ...
        self.k_proj = TensorTrainLinear(
            in_features=self.d_model,
            out_features=config.num_key_value_heads * self.d_qk,
            tt_rank=config.tt_rank,
            tt_cores=config.tt_cores,
            tt_rank_ratio=config.tt_rank_ratio,
            pg=tp_pg,
        ...
        self.v_proj = TensorTrainLinear(
            in_features=self.d_model,
            out_features=config.num_key_value_heads * self.d_v,
            tt_rank=config.tt_rank,
            tt_cores=config.tt_cores,
            tt_rank_ratio=config.tt_rank_ratio,
            pg=tp_pg,
        ...
        # Tensor-Train O projection
        self.o_proj = TensorTrainLinear(
            in_features=config.num_attention_heads * self.d_qk,
    ...
    def forward(
        ...
        # Apply Tensor-Train Q, K, V projections
        query_states = self.q_proj(hidden_states)  # [seq_length, batch_size, num_attention_heads * d_qk]
        key_states = self.k_proj(hidden_states)    # [seq_length, batch_size, num_key_value_heads * d_qk]
        value_states = self.v_proj(hidden_states)  # [seq_length, batch_size, num_key_value_heads * d_v]

        q_length, batch_size, _ = query_states.shape
        query_states = query_states.view(batch_size, q_length, self.n_local_q_heads, self.d_qk)  # [batch_size, q_length, n_local_q_heads, d_qk]
        key_states = key_states.view(batch_size, q_length, self.n_local_kv_heads, self.d_qk)     # [batch_size, q_length, n_local_kv_heads, d_qk]
        value_states = value_states.view(batch_size, q_length, self.n_local_kv_heads, self.d_v)  # [batch_size, q_length, n_local_kv_heads, d_v]
        ...
        if store is not None:  # Inference case
            ...
            query_states = query_states.view(
                batch_size * q_length, self.num_attention_heads, self.d_qk
            )  # [batch_size * q_length, self.n_heads, d_qk]

            key_states = key_states.view(
                batch_size * kv_length, self.num_key_value_heads, self.d_qk
            )  # [batch_size * kv_length, self.n_heads, d_qk]
            value_states = value_states.view(
                batch_size * kv_length, self.num_key_value_heads, self.d_v
            )  # [batch_size * kv_length, self.n_heads, d_v]
        ...
        attention_output = (
            attention_output.contiguous().view(batch_size, q_length, self.num_attention_heads * self.d_v).transpose(0, 1)
        )
        torch.cuda.nvtx.range_push("o_proj")
        # Apply Tensor-Train output projection
...
class Embedding(nn.Module, AttachableStore):
    def __init__(self, tp_pg: dist.ProcessGroup, config: TTLlamaConfig, parallel_config: Optional[ParallelismArgs]):
        super().__init__()
...
class VanillaTTLlamaForTraining(NanotronModel):
    def __init__(
        self,
        config: TTLlamaConfig,
        parallel_context: ParallelContext,
    ...
    def init_model_randomly(self, config: Config):
        ...
        for param_name, param in model.named_parameters():
            ...
            # Skip TensorTrainLinear parameters - they are already initialized in __init__
            if isinstance(module, TensorTrainLinear):
                # Skip bias and any other TensorTrainLinear parameters
                initialized_parameters.add(full_param_name)
                continue
            elif isinstance(module, nn.ParameterList):
                # Check if parent module is TensorTrainLinear (for cores)
                parent_module_name = module_name.rsplit(".", 1)[0] if "." in module_name else ""
                if parent_module_name:
                    parent_module = model.get_submodule(parent_module_name)
                    if isinstance(parent_module, TensorTrainLinear):
                        initialized_parameters.add(full_param_name)
                        continue
...
def get_flops(
    ...
    """Counts flops in tensor-train llama model
    Args:
    ...
    # decoder
    # self attention
    ## qkv projection (each is TT: hidden -> tt_rank -> output)
    decoder_qkv_proj_flops_fwd = (
        2 * num_layers * batch_size * seq_len * ((hidden_size) * tt_rank + (tt_rank) * num_heads * hidden_size_per_head)
        + 2 * 2 * num_layers * batch_size * seq_len * ((hidden_size) * tt_rank + (tt_rank) * num_key_value_heads * hidden_size_per_head)
    )
    ## qk logits
    ...
    ## attn out (TT: attn_output -> tt_rank -> hidden)
    decoder_attn_out_flops_fwd = (
        2 * num_layers * batch_size * seq_len * ((num_heads * hidden_size_per_head) * tt_rank + (tt_rank) * hidden_size)
    )
    # FF
    ## gate and up projections (each TT: hidden -> tt_rank -> intermediate)
    decoder_ffn_1_flops_fwd = 2 * 2 * num_layers * batch_size * seq_len * ((hidden_size) * tt_rank + (tt_rank) * ffn_hidden_size)
    ## down projection (TT: intermediate -> tt_rank -> hidden)
    decoder_ffn_2_flops_fwd = 2 * num_layers * batch_size * seq_len * ((ffn_hidden_size) * tt_rank + (tt_rank) * hidden_size)
