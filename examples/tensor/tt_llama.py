...
class TensorTrainLinear(nn.Module):
    """Tensor-Train decomposition for linear layers.

    Decomposes a linear layer W: in_features -> out_features into:
    W ≈ W_out @ W_in, where W_in: in_features -> tt_rank, W_out: tt_rank -> out_features

    This is the simplest TT decomposition with minimal new parameters.
    """
    def __init__(
        self,
        in_features: int,
        ...
        # First TT core: in_features -> tt_rank
        self.tt_core_in = TensorParallelColumnLinear(
            in_features,
            tt_rank,
            pg=pg,
            mode=mode,
        ...
        # Second TT core: tt_rank -> out_features
        self.tt_core_out = TensorParallelRowLinear(
            tt_rank,
            out_features,
            pg=pg,
            mode=mode,
    ...
    def forward(self, hidden_states):
        # Apply first TT core
        hidden_states = self.tt_core_in(hidden_states)
        # Apply second TT core
        hidden_states = self.tt_core_out(hidden_states)
...
class MLP(nn.Module):
    def __init__(
        ...
        # Tensor-Train MLP: Use TT decomposition for gate and up projections
        # Gate projection: hidden_size -> intermediate_size (via TT-rank)
        self.gate_proj = TensorTrainLinear(
            ...
            tp_recompute_allgather=parallel_config.tp_recompute_allgather if parallel_config is not None else True,
        )

        # Up projection: hidden_size -> intermediate_size (via TT-rank)
        ...
        # Down projection: intermediate_size -> hidden_size (via TT-rank)
    ...
    def forward(self, hidden_states, rstd=None):  # [seq_length, batch_size, hidden_dim]
        ...
        hidden = down_proj(act_fn(gate_proj(x)) * up_proj(x))
        where each projection uses TT decomposition.
        """
        # Apply gate projection with TT decomposition
        # [seq_length, batch_size, intermediate_size]
        gate_output = self.gate_proj(hidden_states)
        gate_output = self.act_fn(gate_output)

        # Apply up projection with TT decomposition
        # [seq_length, batch_size, intermediate_size]
        ...
        # Apply down projection with TT decomposition
...
class CausalSelfAttention(nn.Module, AttachableStore):
    ...
    def forward(
        ...
        # [seq_length, batch_size, num_attention_heads * d_qk]
        query_states = self.q_proj(hidden_states)
        # [seq_length, batch_size, num_key_value_heads * d_qk]
...
def get_flops(
    ...
    # decoder
    # self attention
    ## qkv projection (each is TT: hidden -> rank -> output)
